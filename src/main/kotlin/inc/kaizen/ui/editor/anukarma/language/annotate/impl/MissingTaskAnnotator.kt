...
class MissingTaskAnnotator: IAnnotator {

    override fun annotate(element: PsiElement, holder: AnnotationHolder) {
        if(element.elementType == IAnukarmaLanguageFileType.IDENTIFIER &&
            element.parent.elementType == IAnukarmaLanguageFileType.TASK_DEPENDENCY_ID) {
            if(!findTaskById(element)) {
                holder.newAnnotation(HighlightSeverity.ERROR, "Missing task for configured ID")
                    .range(element.textRange)
                    .problemGroup { "Anukarma" }
                    .tooltip("The feature with ID '${element.text}' does not exist")
                    .highlightType(ProblemHighlightType.ERROR)
                    // ** Tutorial step 19. - Add a quick fix for the string containing possible properties
                    .withFix(AnukarmaCreatePropertyQuickFix("Create New Task with ID ${element.text}"))
                    .create()
            }
        }
    }
    ...
    private fun findTaskById(element: PsiElement): Boolean {
        return element.containingFile.text.contains("task(\"${element.text}\")")
    }
}